...
// Desplazamiento absoluto que deja centrada una jugada. Se mide sobre la
// pantalla, así que no depende de márgenes ni de cómo esté colocado todo.
function offsetCentering(ply) {
  const chip = moveChipEls[ply];
  if (!chip) return stripOffset;
  const strip = moveStripEl.getBoundingClientRect();
  const c = chip.getBoundingClientRect();
  return stripOffset + (strip.left + strip.width / 2) - (c.left + c.width / 2);
}

function stripBounds() {
  const last = moveChipEls.length - 1;
  if (last < 0) return { min: 0, max: 0 };
  return { max: offsetCentering(0), min: offsetCentering(last) };
}
...
(function enableStripControl() {
  ...
  let holdTimer = null, edgeTimer = null, inertia = null, pointerId = null;
  let scrubbing = false, dragging = false;
  let startX = 0, lastX = 0, lastT = 0, velocity = 0;
  let startPly = 0, startOffset = 0, pxPerPly = 56, edgeDir = 0;
  ...
  // En el extremo el dedo ya no puede seguir, así que la partida continúa
  // sola mientras se mantenga pulsado ahí.
  function updateEdge(x) {
    const r = moveStripEl.getBoundingClientRect();
    const dir = x > r.right - EDGE_ZONE ? 1 : (x < r.left + EDGE_ZONE ? -1 : 0);
    ...
    edgeTimer = setInterval(() => {
      ...
      if (next < 0 || next > gameState.positions.length - 1) { stopEdge(); return; }
      goToPly(next);
      startPly = currentPly;     // el recorrido sigue desde aquí
      startX = lastX;
    }, EDGE_MS);
  }
  ...
  moveStripEl.addEventListener('pointermove', (e) => {
    ...
    // El contenido va donde va el dedo: mismo signo, sin vueltas.
  ...
  moveStripEl.addEventListener('pointerup', () => {
    ...
    endGesture();
    if (wasScrubbing) settleAfterScrub();
  ...
  // Al girar el iPad cambia el ancho: se recoloca la jugada actual.
  window.addEventListener('resize', () => {
    if (gameState && moveChipEls.length) {
